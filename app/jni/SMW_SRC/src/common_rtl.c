...
void StateRecorder_Save(StateRecorder *sr, SDL_RWops *rwops, bool saving_with_bug) {
  uint32 hdr[16] = { 0 };
  ...
  SDL_RWwrite(rwops, savest.array.data, savest.array.size, 1);
...
// Allow us to read from external path
...
void RtlSaveSnapshot(const char *filename, bool saving_with_bug) {
  ...
  rwops = SDL_RWFromFileInExternal(filename, "wb");

  RtlApuLock();
  RtlSaveMusicStateToRam_Locked();
  StateRecorder_Save(&state_recorder, rwops, saving_with_bug);
  RtlApuUnlock();
  SDL_RWclose(rwops);
}
...
void RtlSaveLoad(int cmd, int slot) {
  ...
  printf("*** %s slot %d: %s\n",
         cmd == kSaveLoad_Save ? "Saving" : cmd == kSaveLoad_Load ? "Loading" : "Replaying", slot, name);
  if (cmd != kSaveLoad_Save) {
    rwops = SDL_RWFromFileInExternal(name, "rb");
    if (rwops == NULL) {
      printf("Failed fopen: %s\n", name);
      return;
    }
    ...
    RtlSaveSnapshot(name, false);
    SDL_AndroidShowToast("State saved", 0, 0, 0, 0);
  }
}
...
void RtlReadSram(void) {
  ...
  SDL_RWops *stream = SDL_RWFromFileInExternal(filename, "rb");
  if (stream) {

    if (SDL_RWread(stream, g_sram, 1, g_sram_size) != g_sram_size)
...
void RtlWriteSram(void) {
  ...
  SDL_RWops *stream = SDL_RWFromFileInExternal(filename, "wb");
  if (stream) {
    SDL_RWwrite(stream, g_sram, 1, g_sram_size);
    SDL_RWclose(stream);
  } else {
